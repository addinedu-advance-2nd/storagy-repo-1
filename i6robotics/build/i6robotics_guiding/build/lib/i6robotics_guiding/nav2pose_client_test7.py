...
from guiding_services.srv import SetGoalChoice  # 서비스 메시지 임포트
...
class NavigateToPoseClient(Node):
    def __init__(self):
        ...
        # 서비스 서버 생성
        self.srv = self.create_service(SetGoalChoice, 'set_goal_choice', self.handle_set_goal_choice)

        self.get_logger().info('NavigateToPoseClient initialized with service /set_goal_choice')
    ...
    def handle_set_goal_choice(self, request, response):
        choice = request.choice
        if choice == 0:
            self.get_logger().info("Received shutdown command (choice=0).")
        ...
        if 1 <= choice <= len(self.goal_poses):
            ...
            response.success = True
            response.message = f"Goal #{choice} sent successfully."
        else:
            response.success = False
            response.message = f"Invalid choice: {choice}. Valid range is 1 to {len(self.goal_poses)}."
